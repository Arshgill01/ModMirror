...
| Runtime verification matrix | runtime verified for safe status paths | W13 `GET /api/runtime-verification`, `runtimeVerification.test.ts`, W13 playtest `v0.0.1.71`, post-W34 playtest through `v0.0.1.122`, W17 fallback observation on `v0.0.1.123`, access-guard playtest-ready build `v0.0.1.126`, permission diagnostic playtest `v0.0.1.129`, and the Settings manual runtime-event recorder for moderator-observed proof. | Keep truth labels current after runtime changes. |
...
| Current moderator context | partial | `context.username`, `reddit.getCurrentUser()` in smoke; W29 `v0.0.1.122` `/api/health` returned `BrightyBrainiac` for authenticated Devvit Web request context. Protected `/api/access/diagnostics` is locally verified, and the Devvit WebView Settings diagnostic on `v0.0.1.129` returned current moderator permission `all`. A local visibility helper now maps only `all` to full moderator visibility; other permissions remain aggregate-only. | Verify lower-permission moderator roles before expanding the full-access-only per-mod/admin gate. |
...
| AI advisory layer | disabled/type-only | W10 `aiAdvisory.ts`, `/api/ai/capabilities`, `/api/ai/advisory`, mocked-provider tests; `AI_PRIVACY_READINESS.md`; Devvit docs/typings show server fetch and secret settings support | Keep disabled until provider config, HTTP permission, secrets, provider terms/privacy review, and runtime proof are complete. |
| Team delivery receipts | runtime verified for manual/skipped receipts | W11 `teamDelivery.ts`, `teamDelivery.test.ts`, Redis keys `delivery:receipts` and `delivery:receipt:{id}`; post-W34 verified manual-ready and skipped Mod Discussion draft receipt persistence; scheduler confirmations are locally guarded as skipped. | Real Mod Discussion send and scheduler jobs remain disabled. |
| Non-mod access blocking | locally guarded, runtime unverified | `moderatorAccess.ts` denies protected API routes when `getModPermissionsForSubreddit` is unavailable, fails, or returns no permissions; unit tests cover denial cases. Current moderator diagnostic returned permission `all`, but no true non-mod account was used. Future per-mod/manage-level visibility is additionally full-access-only for `all`. | Runtime-verify with a true non-mod account and record exact failure shape. |
