...
| Runtime verification matrix | runtime verified for safe status paths | W13 `GET /api/runtime-verification`, `runtimeVerification.test.ts`, W13 playtest `v0.0.1.71`, post-W34 playtest through `v0.0.1.122`, and W17 fallback observation on `v0.0.1.123` | Keep truth labels current after runtime changes. |
