...
| Full scan persistence and history | W05/W06/W07 reports and post-W34 live scan/replay/correction proof record persisted scan-derived flows. V4 Wave 23 WebView proof on `v0.0.2.6` recorded live deep scan output above one page (`120` actions, requested limit `250`, page size `100`); exact API/page-cursor trace remains a follow-up before weakening conservative warnings. | Satisfied for implemented scope; follow-up proof open |
...
| Public comment delivery proof plan | `docs/operational-overhaul/PUBLIC_COMMENT_DELIVERY_TEST_PLAN.md` defines the approval, throwaway-content, identity, distinguish/sticky, and before/after-removal proof gates before any public-comment delivery path can be enabled. | Satisfied as plan only; runtime proof open |
| Private delivery proof plan | `docs/operational-overhaul/PRIVATE_DELIVERY_TEST_PLAN.md` defines the approval, consenting-recipient, sender-identity, visibility, and deprecated-API avoidance gates before private-message or modmail delivery can be marked runtime-verified. | Satisfied as plan only; runtime proof open |
