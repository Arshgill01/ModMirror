...
| Redis sorted-set ordering | runtime failed | Devvit playtest `v0.0.1.131` ran Settings `Run Redis ZSET`; the WebView reported `Redis sorted-set smoke order mismatch: expected newest, middle, oldest, observed .` and the runtime capability matrix recorded `redis-zset-ordering` as failed. | Rerun after the diagnostic reports add count, cardinality, row count, observed scores, and score checks; do not treat reverse-rank sorted-set reads as runtime verified until `observedOrder` matches `expectedOrder`. |
